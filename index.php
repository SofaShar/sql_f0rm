    ...
    if (empty($errors)) {
        try {
            ...
            if ($editMode && isset($_SESSION['user_id'])) {
                ...
                $pdo->commit();
                // При обновлении редирект не нужен, но чтобы очистить GET, сделаем мягкий редирект
...
// ------ ЧТЕНИЕ СООБЩЕНИЙ И ДАННЫХ ИЗ СЕССИИ ------
$successMessage = $_SESSION['success_message'] ?? '';
unset($_SESSION['success_message']);

$generatedLogin = $_SESSION['generated_login'] ?? '';
$generatedPassword = $_SESSION['generated_password'] ?? '';
unset($_SESSION['generated_login'], $_SESSION['generated_password']);

// ------ ЧТЕНИЕ COOKIES ДЛЯ ОШИБОК И СТАРЫХ ВВОДОВ (НЕАВТОРИЗОВАННЫЕ) ------
$errors = [];
$oldInput = [];
if (!$editMode) {
...
<div class="container">
    ...
    <?php if ($generatedLogin && $generatedPassword): ?>
        <div class="credentials">
            ...
            Логин: <?= htmlspecialchars($generatedLogin) ?><br>
            Пароль: <?= htmlspecialchars($generatedPassword) ?>
        </div>
    <?php endif; ?>
